...
# Updated defaults
TOP_K = int(os.getenv("TOP_K", "15"))  # Changed to 15
...
try:
    ...
    # Initialize reranker model (lightweight and fast)
    ...
    RERANKER_MODEL = None
    logger.warning(
        "sentence-transformers not available - reranking disabled. Install with: pip install sentence-transformers"
    )
...
class PineconeFuzzyCache:
    ...
    def get_or_load(
        self, namespace: str, index, max_samples: int = 500
    ) -> Dict[str, List[str]]:
        """
...
class HybridRAGQueryEngine:
    """
    ENHANCED: Optimized RAG Engine with Pinecone fuzzy matching and reranking
    """
    ...
    def __init__(
        ...
        self.metadata_cache = _pinecone_cache.get_or_load(
            namespace=namespace, index=self.index
        )
        ...
        if self.use_reranker:
            logger.info("Reranker enabled for improved accuracy")
                ...
                for j in range(min(i + 4, len(words)), i, -1):  # Up to 4-word phrases
                    ...
                    if result:
                        ...
                        if score >= threshold - 5:
                            ...
                            logger.info(
                                f"Fuzzy matched doctor: '{phrase}' -> '{matched_name}' (score: {score})"
                            )
        ...
        for idx, word in enumerate(words):
            ...
            if self.metadata_cache["specialties"]:
                ...
                if result:
                    ...
                    logger.info(
                        f"Fuzzy matched specialty: '{clean_word}' -> '{matched_specialty}' (score: {score})"
                    )
                    continue
            ...
            if self.metadata_cache["departments"]:
                ...
                if result:
                    ...
                    logger.info(
                        f"Fuzzy matched department: '{clean_word}' -> '{matched_dept}' (score: {score})"
                    )

        if corrections:
            logger.info(
                f"Query correction: '{query}' -> '{corrected_query}' ({len(corrections)} changes)"
            )

        return corrected_query, corrections

    def fuzzy_match_metadata_field(
        self, field_name: str, query_value: str, top_k: int = 5, threshold: int = 75
    ) -> List[Dict]:
        """
    ...
    def detect_id_in_query(self, query: str) -> Dict[str, Any]:
        ...
        if match := self.patterns["record_id"].search(query):
            ...
            if FUZZY_AVAILABLE and self.metadata_cache["record_ids"]:
                ...
                if fuzzy_matches:
                    logger.info(
                        f"Using fuzzy matched record_id: {fuzzy_matches[0]['value']}"
                    )
        ...
        if match := self.patterns["phone_formatted"].search(query):
            ...
            if 10 <= phone_digits <= 14:
                ...
                if FUZZY_AVAILABLE and self.metadata_cache["phones"]:
                    ...
                    if fuzzy_matches:
                        logger.info(
                            f"Using fuzzy matched phone: {fuzzy_matches[0]['value']}"
                        )
    ...
    def retrieve_by_id(
        ...
        """
        Enhanced exact ID matching with fuzzy fallback and reranking
        """
        ...
        try:
            ...
            if results.matches:
                ...
                if self.use_reranker:
                    query_text = f"Find record with {id_type} {id_value}"
                    docs = self._rerank_documents(
                        query_text, docs, top_n=min(5, len(docs))
                    )
            ...
            if FUZZY_AVAILABLE:
                fuzzy_matches = self.fuzzy_match_metadata_field(
                    field_name=id_type, query_value=id_value, top_k=3, threshold=80
                )
                ...
                if fuzzy_matches:
                    ...
                    if all_docs:
                        ...
                        if self.use_reranker:
                            query_text = f"Find record with {id_type} {id_value}"
                            all_docs = self._rerank_documents(
                                query_text, all_docs, top_n=min(top_k, len(all_docs))
                            )
                        return all_docs[:top_k]

            # Final fallback: semantic search
            return self.retrieve_semantic(
                f"information about {id_type} {id_value}", top_k=top_k
            )
    ...
    def retrieve_semantic(
        ...
        """
        ENHANCED: Semantic search with automatic query correction and reranking
        """
        if top_k is None:
            top_k = self.top_k
        ...
        if corrections:
            logger.info(
                f"Using corrected query for semantic search: '{corrected_query}'"
            )
            query = corrected_query

        try:
            # Use MMR for diversity - fetch more for reranking
            ...
            retriever = self.vectorstore.as_retriever(
                ...
                search_kwargs={
                    "k": (
                        top_k if not self.use_reranker else top_k * 2
                    ),  # Get more docs for reranking
                    "fetch_k": fetch_k,
                    "lambda_mult": 0.7,
                },
            )
            ...
            if self.use_reranker and docs:
                docs = self._rerank_documents(query, docs, top_n=top_k)

            logger.info(
                f"Found {len(docs)} semantic matches (reranked: {self.use_reranker})"
            )
            return docs
    ...
    def hybrid_retrieve(
        ...
        """
        ENHANCED: Hybrid retrieval with fuzzy correction and reranking
        """
        if top_k is None:
            top_k = self.top_k
